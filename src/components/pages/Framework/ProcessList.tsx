...
import { Process, RequiredProcess } from "../../../redux/frameworkSlice";
...
export default function ProcessList({
  ...
  return (
    <div className={classes.tableBody}>
      <Table>
        <TableBody>
          {requiredProcs.map((row) => (
            <Row
              key={`required-procs-${row.name}`}
              row={row}
    ...
    <>
      <TableRow key={`${row.name}-tablerow`}>
        <TableCell
          ...
          align="left"
        >
          <strong>{row.name}</strong> - {row.processes.length} Processes
        </TableCell>
      ...
      <TableRow key={`${row.name}-tablerow-collapsible`}>
        <TableCell style={{ padding: 0, borderTop: 0 }} colSpan={5}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            ...
            >
              <SubList procs={row.processes} title={row.name} />
            </div>
          </Collapse>
        </TableCell>
      </TableRow>
...
function SubList({ procs, title }: { procs: Process[]; title: string }) {
  const classes = useStyles();
